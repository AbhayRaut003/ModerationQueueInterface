...
const moderationSlice = createSlice({
  ...
  initialState,
  reducers: {
    setFilter: (state, action: PayloadAction<'pending' | 'approved' | 'rejected'>) => {
    ...
    batchApprove: (state, action: PayloadAction<string[]>) => {
      ...
      const previousStates: Array<{ id: string; previousStatus: Post['status'] }> = [];
    ...
    navigateModal: (state, action: PayloadAction<'next' | 'prev'>) => {
      ...
      const filteredPosts = state.posts.filter(post => post.status === state.filter);
      const currentIndex = filteredPosts.findIndex(post => post.id === state.currentPost);
      
      if (action.payload === 'next' && currentIndex < filteredPosts.length - 1) {
    ...
    undoAction: (state, action: PayloadAction<string>) => {
      const undoIndex = state.undoStack.findIndex(item => item.id === action.payload);
      if (undoIndex > -1) {
